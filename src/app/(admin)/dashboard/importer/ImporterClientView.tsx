...
export default function ImporterClientView({ initialResults, initialQuery }: { initialResults: any[], initialQuery: string }) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (!query) return;
    setSearching(true);
    router.push(`/dashboard/importer?q=${encodeURIComponent(query)}`);
    // Note: The loading state will be cleared when the page finishes navigation or if we do it in a useEffect based on initialQuery.
    // However, since it's server-driven, Next.js streaming will show `loading.tsx` automatically.
    // Wait, next navigation might take time, so we keep `searching` true until the page loads.
  };

  // Turn off searching spinner when new results arrive
  useEffect(() => {
    setSearching(false);
  ...
  return (
    <div className="animate-fade-in">
      ...
      {initialResults.length === 0 && !searching && (
        <div className="text-center py-[100px] text-[#94A3B8]">
          <div className="text-[64px] opacity-20 mb-6">
            <i className="fas fa-search-plus"></i>
          </div>
          <h3 className="text-lg font-extrabold text-[#64748B] mb-2">{initialQuery ? 'No Products Found' : 'Ready to grow your catalog?'}</h3>
          <p className="max-w-[400px] mx-auto">{initialQuery ? 'We couldn\'t find any products matching your search.' : 'Search the CJ global network using product titles, SKUs, or direct URLs to import best-sellers instantly.'}</p>
        </div>
      )}
